<c:if test="${!empty(tarea)}">
    <div class="mb-3">
        ...
        <input type="text" class="form-control" name="id" id="id" value="${tarea.idTarea}" readonly>
    </div>
</c:if>
...
<div class="mb-3">
    ...
    <select class="form-select" name="estado" id="estado" required>
        <c:forEach items="${['Pendiente', 'En Proceso', 'Finalizada']}" var="estado" varStatus="status">
            <option value="${status.index + 1}" ${tarea.estado == status.index + 1 ? 'selected' : ''}>${estado}</option>
        </c:forEach>
    </select>
</div>
</form>